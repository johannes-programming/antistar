...
test: beautiful
	conda run -n base python make/env.py test_antistar --python=3.11 --recreate >/dev/null;
	conda run -n test_antistar pip install -e . >/dev/null;
	conda run -n test_antistar python run_tests.py;
	conda run -n test_antistar pip install mypy >/dev/null;
	conda run -n test_antistar python -m mypy --exclude build --exclude dist --strict .;
	conda run -n test_antistar python -m mypy --strict -p antistar;
